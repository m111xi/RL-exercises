...
class DQNAgent(AbstractAgent):
    ...
    def epsilon(self) -> float:
        ...
        self.epsilon_decay = self.epsilon_final + (self.epsilon_start - self.epsilon_final) * np.exp(-self.total_steps / self.epsilon_decay)
    ...
    def update_agent(
        ...
        with torch.no_grad():
            target = r + self.gamma * (1 - mask) * self.target_q(s_next).max(1)[0]
